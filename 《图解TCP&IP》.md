   ...
   * ICMPv6:
   ...
   7. 但是：
      ...
      * 捎带应答：TCP的确认应答和回执数据通过一个包发送。
      ...
      1. UDP-Lite:扩展UDP机能的一种传输层协议，只针对不允许发生错误的部分进行校验和的检查。
      2. SCTP：用于进行通信的应用之间发送众多较小消息的情况。支持多重宿主以及设定多个IP的特点，可以管理多个IP使其同时进行通信。
      3. DCCP：与UDP一样，不能提供发送数据的可靠性传输，但是能够根据网络拥堵情况进行拥塞控制。
      ...
      | ---- | ---------- | ------------------------------------------------------------ |
      | 1    | 源端口号   | 16位，该字段是可选项                                         |
      | 2    | 目标端口号 | 16位，表示接收端端口号                                       |
      | 3    | 包长度     | UDP首部的长度跟数据的长度之和                                |
      | 4    | 校验和     | 为了提供可靠的UDP首部和数据而设计；接收主机在收到UDP数据报之后，从IP首部获知IP地址信息构造UDP伪首部，再进行校验和计算。另外，UDP中也有可能不用校验和，此时协议处理的开销就会降低。 |
      ...
      | 1    | 源端口号   | 16位，发送端端口号                                           |
      | 2    | 目标端口号 | 16位，目标端口号                                             |
      | 3    | 序列号     | 发送数据的位置，每发送一次数据，就累加一次该数据字节数的大小。在建立连接和断开连接时发送的SYN包和FIN包虽然不携带数据，但是也会作为一个字节增加对应的序列号。 |
      | 4    | 确认应答号 | 32位，下一次应该收到的数据的序列号。实际上，它是指已收到确认应答号前一位为止的数据。发送端收到这个确认应答以后可以认为在这个序列号以前的数据都已经被正常接收。 |
      | 5    | 数据偏移   | 表示TCP所传输的数据部分应该从TCP包的哪个位开始计算，当然也可以把它看做TCP首部的长度。 |
      | 6    | 保留       | 为了以后扩展时使用                                           |
      | 7    | 控制位     | 共8位，从左至右分别为CWR\ECE\URG\ACK\PSH\RST\SYN\FIN         |
      | 8    | 窗口大小   | 通知从相同TCP首部的确认应答号所指位置开始能够接收数据的大小。 |
      | 9    | 校验和     | TCP的校验和无法关闭                                          |
      | 10   | 紧急指针   | 本报文段中紧急数据的指针。如何处理紧急数据属于应用的问题。一般在暂时中断通信的情况下使用。 |
      | 11   | 选项       | 用于提高TCP的传输性能。                                      |

      * 使用校验和的目的是什么？

      在有噪声干扰的通信途中如果出现位错误，可以由数据链路的FCS检验出来。但是，相比检查噪声影响导致的错误，TCP或UDP的校验和更是一种进行路由器内存故障或程序漏洞导致的数据是否被破坏的检查。

      **在互联网中发送数据包要经由好多路由器，一旦在发送途中的某一个路由器发生故障，经过此路由器的包、协议首部或数据就极有可能被破坏。**即使在这种情况下，TCP或UDP如果能够提供校验和计算，也可以判断协议首部和数据是否被破坏。

      

      ## 七、路由协议

      互联网是由路由器连接的网络组合而成的。路由器根据路由控制表转发数据包，根据所收到的数据包中目标主机的IP地址与路由控制表的比较得出下一个应该接收的路由器。

      1. 静态路由：事先设置好路由器和主机，并将路由信息固定。每增加一个网络，就需要将这个新被追加的网络信息设置在所有的路由器上。
      2. 动态路由：管理员必须设置好路由协议。路由器为了能够像这样定期相互交换必要的路由控制信息，会与相邻的路由器之间互发消息。这些互换消息会给网络带来一定程度的负荷。

      * 动态路由的基础：会给相邻路由器发送自己已知的网络连接信息，而这些信息又像接力一样依次传递给其他路由器，甚至整个网络都了解时，路由控制表也就制作完成了。

      ### 1 路由控制范围

      * 人们根据路由控制的范围使用IGP和EGP两种类型的路由协议。

      * IP地址分为网络部分和主机部分，他们有各自的分工。EGP和IGP的关系与IP地址网络部分和主机部分的关系有相似之处。就像根据IP地址中的网络部分在网络之间进行路由选择、根据主机部分在链路内部进行主机识别一样。可以根据EPG在网络之间进行**路由选择**，也可以根据IGP在区域网络内部进行**主机识别**。

      * 路由协议被分为EGP和IGP两个层次。没有EGP就不可能有世界上各个不同组织机构之间的通信。没有IGP机构内部也就不能进行通信。

      ### 2 路由算法

      * 距离向量算法：根据距离和方向决定目标网络或目标主机位置。当网络构造变得复杂时，获得稳定路由需要花费一定时间，也极易发生路由循环问题。
      * 链路状态算法：在了解网络整体连接状态的基础上生成路由控制表的方法。管理和处理代理信息需要高速CPU处理能力和大量的内存。
      ...
      * 距离的单位为“跳数”。跳数指所经过的路由器个数。根据距离向量生成距离向量表，再抽出较小的路由生成最终的路由控制表。

      * 路由变更时，RIP将自己所知道的路由信息定期进行广播；一旦认为网络被断开，数据将无法流过此路由器，其他路由器也就可以得知网络已经断开。

      * 在有环路的情况下，反向的回路会成为迂回的通道，路由信息会不断地被循环往复地转发。需要很长时间才能产生正确的路由信息。为了尽可能解决这个问题，人们提出了“毒性逆转”和“触发更新”。

        * 毒性逆转：当网络中发生链路断开时，不是不再发送这个消息，而是将这个无法通信的消息传播出去。即发送一个距离为16的消息。
        * 触发更新：路由信息发生变化时，不等待30秒而是立即发送出去。

        有了这两种方法，在链路不通的情况下，可以迅速传播消息以使路由信息尽快收敛。

      * RIP2：

        1. 使用多播，摒弃了广播，减少了网络流量。
        2. 支持子网掩码
        3. 可以使用逻辑上独立的多个RIP
        4. 外部路由标志
        5. 身份验证秘钥
      ...
      采用链路状态类型，即使网络中由环路，也能够进行稳定的路由控制。

      * OSPF支持子网掩码，曾经在RIP中无法实现的可变长度子网构造的网络路由控制成为现实。
      * RIP的路由选择，要求途中所经过的路由器个数越少越好。与之相比，OSPF可以给每条链路赋予一个权重，并始终选择一个权重最小的路径作为最终路由。也就是说，OSPF以每个链路上的代价为度量标准，始终选择一个总的代价和最小的路径。
      * RIP是选择路由器个数最少的路径，而OSPF是选择总的代价较小的路径。
      * 确定一个指定路由器，并以它为中心交换路由信息。
      * RIP中包的类型只有一种，它利用路由控制信息，一边确认是否连接了网络，一边传送网络信息。但是这种方式有一个严重的缺点。那就是网络的个数越多，每次所要交换的路由控制信息就越大。而且当网络已经出于比较稳定的、没有什么变化的状态时，还是要定期交换相同的路由控制信息，这在一定程度上浪费了网络带宽。而在OSPF中，根据作用可以分为5中不同类型的包：
        1. 问候:确认相邻路由器、确定指定路由器；
        2. 数据库描述：链路状态数据库的摘要信息；
        3. 链路状态请求：请求从数据库中获取链路状态信息；
        4. 链路状态更新：更新链路状态数据库中的链路状态信息；
        5. 链路状态确认应答：链路状态信息更新的确认应答。
      * 通过发送**问候包**确认是否连接。每个路由器为了同步路由控制信息，利用**数据库描述包**相互发送路由摘要信息和版本信息。如果版本比较老，则首先发出一个**链路状态请求包**请求路由控制信息，然后由**链路状态更新包**接收路由状态信息，最后再通过**链路状态确认包**通知大家本地已经接收到路由控制信息。**有了这个机制，OSPF不仅可以大大地减少网络流量，还可以达到迅速更新路由信息的目的。**
      * LAN中每10秒发送一个HELLO包。如果HELLO包到达，则进行连接是否断开的判断。
      * **将区域分层化进行细分管理**：
        1. OSPF为了减少计算负荷，引入了区域的概念。
        2. 区域是指将连接在一起的网络和主机划分成小组，使一个自治系统内可以拥有多个区域。具有多个区域的自治系统必须要有一个主干区域，并且其他所有自治区域都必须与这个主干区域相连接。
        3. 连接区域与主干区域的路由器称作**区域边界路由器**，区域内部的路由器叫做内部路由器；
        4. **主干路由器**：只与主干区域内连接的路由器叫做主干路由器；
        5. 自治系统（AS）边界路由器：与外部相连接的路由器。
        6. 每个区域内的路由器都持有本区域网络拓扑的数据库。然而，关于区域之外的路径信息，只能从区域边界路由器那里获知它们的距离。区域边界路由器也不会将区域内的链路状态信息全部原样发送给其他区域，只会发送自己到达这些路由器的距离信息，内部路由器所持有的网络拓扑数据库就会明显减小。
        7. 换句话说，内部路由器只了解区域内部的链路状态信息，并在该信息的基础上计算出路由控制表。这种机制不仅可以有效地减少路由控制信息，还能减轻处理的负担。
        8. 作为区域出口的区域边界路由器若只有一个的话叫做末端区域。末端区域内不需要发送区域外的路由信息。它的区域边界路由器将成为默认路径传送路由信息。

      

      ### 5 BCG（Border Gateway Protocol）

      边界网关协议是连接不同组织机构（或自治系统）的一种协议。**只有BCG\RIP和OSPF共同进行路由控制，才能够进行整个互联网的路由控制。**

      * 在RIP和OSPF中利用IP的网络地址部分进行路由控制，然而BCG则需要放眼整个互联网进行路由控制。BCG的最终路由控制表由网络地址和下一站的路由器组成，不过它会根据所要经过的AS个数进行路由控制。
      * ISP、区域网络等会将每个网络域编配成一个个自治系统进行管理。他们为每个自治系统分配一个16比特的AS编号。
      * 有了AS编号的域，就相当于有了自己一个独立的“国家”。AS的代表可以决定AS内部网络运行的相关策略。与其他AS相连的时候，可以像一位“外交官”一样签署合约再进行连接。**正是有了这些不同区域的AS通过签约的相互连接，才有了今天的互联网。**
      * BCG是路径向量协议。
      * BCG扬声器（路由器）为了在AS之间交换BCG信息，必须与所有AS建立对等的BCG连接。
      * **在做路由选择时使用的度量，RIP中表示为路由个数；OSPF中表示为每个子网的成本；BCG则用AS进行度量**。RIP和OSPF本着提高转发效率为母的，考虑到了网络的跳数和网络的带宽。BCG则基于AS之间的合约进行数据包的转发。BCG一般选择AS数最少的路径，不过仍然要遵循各个AS之间签约的细节进行更细粒度的路由选择。
      * BCG不是距离向量型协议，也不是链路状态型协议，而属于路径向量型协议。
      * 作为距离向量型协议的RIP因为无法检测出环路，所以可能发生无限计数的问题。而路径向量型协议由于能够检测出环路，避免了无限计数的问题，所以令网络更容易进入一个稳定的状态。同时，它还支持策略路由的优势。
      * 分布式系统：多个系统协同完成一个特定任务的系统。
      ...
      路由技术基于IP地址中最长匹配原则进行转发，而标记转换技术则对每个IP包都设定一个“标记”，然后根据这个“标记”进行转发。

      * 由于基于标记的转发通常无法在路由器上进行，所以MPLS也就无法被整个互联网采用。

      * MPLS网络中实现MPLS功能的路由器叫做标记转换路由器。

      * MPLS的优势：

        1. 转发速度快。通常，路由器转发IP数据包时，首先要对目标地址和路由控制表中可变长的网络地址进行比较，然后从中选出最长匹配的路径才能进行转发。MPLS则使用固定长度的标记信息，使得处理更加简单，可以通过高速硬件转发。此外，相比互联网中的主干路由器需要保存大量的路由表才能进行处理的现状，MPLS只需要设置必要的几处信息即可，所以要处理的信息也大幅度减少。而且，除了IPv4和IPv6，针对其他协议，MPLS也能够实现高速转发。
        2. 利用标记生成虚拟的路径，并在它上面实现IP等数据报的通信。

        基于这些特点，被称之为“尽力而为”的IP网也可以提供基于MPLS的通信质量控制、贷款保证和VPN等功能。
      ...
      * 需要搭建MPLS网络。
      ...
      应用可以直接使用传输层以下的基础部分。因为开发者只要关心选用哪种应用协议、如何开发即可，而不必担心应用中的数据该以何种方式发送到目标主机等问题。
      ...
      * TELNET可以分为两类基本服务：一是仿真终端功能，二是协商选项机制。TELNET经常用于登录路由器或高性能交换机等网络设备进行相应的设置。
      ...
      * FTP是通过怎样的机制才得以实现文件传输的呢？它使用两条TCP连接：一条用来控制，另一条用于数据（文件）传输。
      * 数据传输的TCP连接通常使用端口20.
      ...
      * 发送人编写好邮件以后，其内容会保存在发送端主机的硬盘中。然后与对端主机建立TCP连接，将邮件发送到对方主机的硬盘中。当发送正常结束后，再从本地硬盘中删除邮件。而在发送过程中一旦发现对端主机因没有插电等原因没有收到邮件时，发送端将等待一定时间再发。
      * 引用一种一直会连接电源的邮件服务器。发送端和接收端通过邮件服务器进行收发邮件。接收端从邮件服务器接收邮件时使用POP3协议。
      * SMTP使用TCP的25号端口，建立一个TCP连接后，在这个连接上进行控制和应答以及数据的发送。客户端以文本的形式发出请求，服务端返回一个3位数字的应答。
      * 个人电脑不可能长时间出于开机状态。只有用户在使用时才开机。在这种情况下，人们希望一开机就能接收到邮件。然而SMTP没有这种处理机制。SMTP的一个不利之处在于它支持的是发送端主机的行为，而不是根据接收端的请求发送邮件。
      * 为了解决这个问题，就引入了POP协议。该协议是一种用于接收电子邮件的协议。发送端的邮件根据SMTP协议将被转发给一直出于插电状态的POP服务器。客户端再根据POP协议从POP服务器接收对方发来的邮件。在这个过程中，为了防止他人盗窃邮件内容，还要进行用户验证。
      * POP与SMTP一样，也是在其客户端和服务器之间建立一个TCP连接完成相应的操作。
      ...
      * 万维网定义了3个重要的概念，他们分别是：访问信息的手段与位置（URI）、信息的表现形式（HTML）、以及信息转发（HTTP）。

      * HTTP默认使用80端口。
      * HTTP有两个版本，HTTP1.0中每一个命令和应答都会触发一次TCP连接和断开；而从HTTP1.1开始，允许在一个TCP连接上发送多个命令和应答。由此，大大地减少了TCP连接的建立和断开操作，从而也提高了效率。

      

      ### 5. SNMP

      * 在TCP/IP的网络管理中可以使用SNMP收集必要的信息，进行网络管理，它是一款基于UDP/IP的协议。
      * SNMP中交互的信息是MIB，MIB相当于SNMP的表示层，它是一种能够在网络上传输的结构。SNMP中可以将MIB值写入代理，也可以从代理中读取MIB值。通过这些操作可以收集冲突的次数和流量统计等信息，可以修改接口的IP地址，还可以进行路由器的启停、设备的启动和关闭等处理。

      

      ### 6.其他应用层协议

      * H.323：由ITU开发用于在IP网上传输音频、视频的一种协议。
      * RTP：采用UDP实现实时的多媒体通信协议需要附加一个表示报文顺序的序列号，还需要对报文发送时间进行管理。
      * MPEG：数字压缩技术。
      * P2P：网络上的终端或主机不经服务器直接一对一相互通信的情况。
      * LDAP：访问目录服务的一种协议，也叫作轻量级目录访问协议。所谓“目录服务“是指网络上存在的一种提供相关资源的数据库的服务。就像DNS为了更简单地对网络上的各个主机进行管理一样，LDAP是为了更简单地管理网络上的各种资源。LDAP定义了目录树的结构、数据格式、命名规则、目录访问顺序和安全认证。**在大规模公司或教育机关中，为了让用户能够使用计算机或某个应用，有必要先进行可否使用计算机或应用的设置。此时如果这些设备和应用了LDAP，并在LDAP服务器中注册了用户，就可以对这些用户是否有效进行判断。**
      ...
      * 防火墙设计的基本思路：暴露给危险的主机和路由器的个数是有限的。
      * 设置防火墙，可以限制从互联网访问的主机个数，将安全的主机和可以爆肉给危险的主机加以区分，只针对后者集中实施安全防护。
      * 对路由器设置只像其发送特定地址和端口的包，也是设置防火墙。还可以监控TCP包首部中的SYN和ACK标志位来实现只允许从内网发起TCP连接。

      ### 2.IDS（入侵检测系统）

      * 数据包符合安全策略，防火墙才会让其通过。即只要与策略符合，就无法判断当前访问是否非法。而IDS正是检查这种已经侵入内部网络进行非法访问的情况，并及时通知给网络管理员。
      * 从功能上看，IDS有定期采集日志、长期监控、通知异常等功能。它可以监控网络上流动的所有数据包。

      ### 3.对称密码体制与公钥密码体制

      ### 4.安全协议 IPsec与VPN

      * 搭建物理专线的成本太高，人们想出了在互联网上构造一个虚拟的私有网络，即VPN。
      * 在构建VPN时，最常被使用的是IPsec。它是指在IP首部的后面追加“封装安全有效载荷”和“认证首部”，从而对此后的数据进行加密，不被盗窃者轻易解读。
      * 在发包的时候加上上述两个首部，可以在收包时根据首部对数据进行解密，恢复成原始数据。

      ### 5. TLS/SSL与HTTPS

      * 信用卡号，银行账号等属于个人的机密信息，Web中可以通过TLS/SSL对HTTP通信进行加密，这种使用TLS/SSL进行加密的HTTP通信就叫HTTPS。

      ### 6. TEEE802.1X

      * 是为了能够接入LAN交换机和无线LAN接入点而对用户进行认证的技术。它只允许被认可的设备才能访问网络。虽然它是一个提供数据链路层控制的规范，但是与TCP/IP关系紧密。
